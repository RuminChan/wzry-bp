...
css_path = r'D:\traeProject\wzry-bp\src\index.css'

with open(css_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Find old BP styles boundaries
start_marker = '/* ===== 阵容面板 ===== */'
end_marker = '/* ===== Modal ===== */'
...
print(f"Old BP styles: char {start_idx} to {end_idx}")

new_styles = """/* ===== BP 3-column layout ===== */
.bp-layout {
  display: grid;
...
# Update mobile media query references
new_content = new_content.replace('.teams-panel', '.bp-layout')
new_content = new_content.replace('.vs-divider', '.center-column')
new_content = new_content.replace('.ban-slots', '.side-bans')
new_content = new_content.replace('.pick-slots', '.side-picks')

with open(css_path, 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Done! Replaced old BP styles with new 3-column layout styles.")
